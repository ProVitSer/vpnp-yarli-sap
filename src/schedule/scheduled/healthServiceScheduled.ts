...
import { HealthCheckMailFormat, ServiceInfo } from "@app/health/types/interfaces";
...
export class HealthServiceScheduledService implements OnApplicationBootstrap{
    ...
    constructor(
        ...
        this.logger.setContext(HealthServiceScheduledService.name);
        this.mailSendInfo = {
    ...
    async onApplicationBootstrap(){
        try {
            ...
            this.logger.error(`Error HealthServiceScheduledService on application start  ${e}`)
        }
    }  
    ...
    async sendScheduled() {
        try{
            ...
            this.logger.error(`Error HealthServiceScheduledService ${e}`)
        }
    }
    ...
    private async sendMailInfo(healthResult: HealthCheckMailFormat){
        try {
            ...
            this.logger.error(`Error send mail from HealthServiceScheduledService ${e}`)
        }
    }
